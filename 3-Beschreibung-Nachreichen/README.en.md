...
More room in **Dungeon Settlers**: ten active skill slots, a separate basic attack and three item quickslots. The familiar character panel stays in place.
...
| Download | What is it for? |
| --- | --- |
| **Extended-Hotbar-Helper-0.1.2-test.zip** | Optional Windows installation/update helper. **Mod 0.3.7 is already included**; no separate mod ZIP needed. |
| **Extended-Hotbar-0.3.7.zip** | Manual installation for your own campaign. |
| **Extended-Hotbar-0.3.7-mit-Testspielstand.zip** | The same manual package plus the optional “10 Slots Testfile” demo save with assigned slots and extra items. |

The two manual packages contain the same mod: do not install both. `SHA256SUMS.txt` provides optional checksums for all three downloads. QA and Frieren are not part of these packages.
