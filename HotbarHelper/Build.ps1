...
$taskCore = @('FrameworkTarget.cs','LosslessJson.cs','Profiles.cs','HotbarKeyProfile.cs','ReleaseInfo.cs','ModLoaders.cs','Operations.cs','LoadStatus.cs','UiText.cs','Updates.cs','UpdateSignature.cs','LocalMods.cs','LocalHelpers.cs','LocalUpdatePreferences.cs','GameDiscovery.cs') | ForEach-Object { Join-Path $PSScriptRoot $_ }
...
if (-not $SkipTests) {
    ...
    & dotnet $taskCompiler @taskCommon @taskLanguages '/target:exe' ('/out:' + $taskTests) @taskResources @taskCore (Join-Path $PSScriptRoot 'LocalOnlyChecks.cs') (Join-Path $PSScriptRoot 'Tests.cs') (Join-Path $PSScriptRoot 'DualLoaderTests.cs') (Join-Path $PSScriptRoot 'UpdateTests.cs') (Join-Path $PSScriptRoot 'LocalModTests.cs') (Join-Path $PSScriptRoot 'LocalUpdateSmokeTests.cs') (Join-Path $PSScriptRoot 'GameDiscoveryTests.cs')
