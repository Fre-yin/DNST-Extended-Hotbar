    ...
    internal static class LocalModTests
    {
        ...
        internal static void Run(Action<string, Action> test, string root)
        {
            ...
            using (var signer = new RSACryptoServiceProvider(4096, new CspParameters(24)))
            {
                ...
                var zero = Updates.ParseVersion("0.0.0"); var version = Updates.ParseVersion("0.3.8");
                ...
                test("local filenames ignore helpers source archives and partial downloads", () => {
                    foreach (var file in new[] { "DNST-Extended-Hotbar-main.zip", "Extended-Hotbar-Helper-0.1.6-test.zip", "Extended-Hotbar-0.3.8.zip.part", "Extended-Hotbar-0.3.8.zip.crdownload" }) Check(LocalMods.FileVersion(file) == null);
                    Check(LocalMods.FileVersion("Extended-Hotbar-0.3.8 (1).zip") == version);
                    Check(LocalMods.FileVersion("Extended-Hotbar-0.3.8-mit-Testspielstand.zip") == version);
                });
                test("local scan is read only and does not search nested folders", () => {
                    ...
                    File.WriteAllBytes(Files.Under(folder, "nested/Extended-Hotbar-0.3.8.zip"), archive("0.3.8"));
                ...
                test("original unsigned mod variants work only with pinned install bytes", () => {
                    var path = Put(store, "0.3.7", Package(null, "0.3.7"));
                    var result = LocalMods.Read(path, CancellationToken.None, publicKey); Check(result.Payload.Count == 3);
                    var demo = Files.Under(store, "Extended-Hotbar-0.3.7-mit-Testspielstand.zip"); File.WriteAllBytes(demo, Package(null, "0.3.7", extra: "Saves/demo.json"));
                ...
                test("future local mod requires the pinned publisher signature", () => {
                    var path = Put(store, "0.3.8", archive("0.3.8")); Check(LocalMods.Read(path, CancellationToken.None, publicKey).Version == version);
                    ...
                    File.WriteAllBytes(path, Package(null, "0.3.8")); Refused(() => LocalMods.Read(path, CancellationToken.None, publicKey));
                });
                test("local scan sorts versions numerically and never offers a downgrade", () => {
                    ...
                    Put(folder, "0.3.9", archive("0.3.9")); Put(folder, "0.3.10", archive("0.3.10"));
                    Check(LocalMods.Scan(folder, zero, CancellationToken.None, publicKey).Version == Updates.ParseVersion("0.3.10"));
                    Check(LocalMods.Scan(folder, Updates.ParseVersion("0.3.11"), CancellationToken.None, publicKey) == null);
                });
                test("equivalent browser duplicates are accepted but conflicting bytes refused", () => {
                    ...
                    var path = Put(folder, "0.3.8", archive("0.3.8")); var duplicate = Files.Under(folder, "Extended-Hotbar-0.3.8 (1).zip");
                    File.Copy(path, duplicate); Check(LocalMods.Scan(folder, zero, CancellationToken.None, publicKey) != null);
                    File.WriteAllBytes(duplicate, Package(signer, "0.3.8", modified: true)); Refused(() => LocalMods.Scan(folder, zero, CancellationToken.None, publicKey));
                });
                test("local update binds version helper minimum and supported game fingerprints", () => {
                    var path = Put(store, "0.3.8", archive("0.3.9")); Refused(() => LocalMods.Read(path, CancellationToken.None, publicKey));
                    foreach (var field in new[] { "purpose", "repository", "minimumHelper", "gameHash", "metadataHash" }) {
                        File.WriteAllBytes(path, Package(signer, "0.3.8", changedField: field)); Refused(() => LocalMods.Read(path, CancellationToken.None, publicKey));
                    }
                });
                test("local update refuses content corruption unsafe paths duplicate entries and symlinks", () => {
                    var path = Files.Under(store, "Extended-Hotbar-0.3.8.zip");
                    foreach (var extra in new[] { "../outside.dll", "Mods\\evil.dll", "Mods/file:ads", ReleaseInfo.Dll, "/root.dll", "folder./file" }) {
                        File.WriteAllBytes(path, Package(signer, "0.3.8", extra: extra)); Refused(() => LocalMods.Read(path, CancellationToken.None, publicKey));
                    }
                    File.WriteAllBytes(path, Package(signer, "0.3.8", corrupt: true)); Refused(() => LocalMods.Read(path, CancellationToken.None, publicKey));
                    File.WriteAllBytes(path, Package(signer, "0.3.8", extra: "link", linked: true)); Refused(() => LocalMods.Read(path, CancellationToken.None, publicKey));
                });
                ...
                test("local confirmation rechecks the exact ZIP and cancellation stops reads", () => {
                    var path = Put(store, "0.3.8", archive("0.3.8")); var offer = LocalMods.Read(path, CancellationToken.None, publicKey);
                    Check(LocalMods.Recheck(offer, CancellationToken.None, publicKey).Count == 3);
                    File.WriteAllBytes(path, Package(signer, "0.3.8", extra: "extra.txt")); Refused(() => LocalMods.Recheck(offer, CancellationToken.None, publicKey));
